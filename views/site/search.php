<?php
	...
	if(isset($_GET['query'])){
		$search = $_GET['query'];
		$showsearch = $prd->product_search($search, $start_index, $per_page);
		extract($showsearch);
	}
	// $showpaginate = $prd->product_search($start_index, $per_page);
?>
	<!-- Start Banner Area -->
					...
					<?php
						if($total_pages > 1){
							...
							if($current_page > 1){
								$pagination .= 
								'
									<a href="?page=search&query='.$search.'&pg='.($current_page-1).'" class="prev-arrow"><i class="fa fa-long-arrow-left" aria-hidden="true"></i></a>
								';
							}
							for($i = max(1, $current_page - 3); $i <= min($current_page + 3, $total_pages); $i++){
								if($i == $current_page){
									$pagination .= '<a href="?page=search&query='.$search.'&pg='.$i.'" class="active">'.$i.'</a>';
								} else {
									$pagination .= '<a href="?page=search&query='.$search.'&pg='.$i.'">'.$i.'</a>';
								}
							}
							if($current_page < $total_pages){
								$pagination .= 
								'
									<a href="?page=search&query='.$search.'&pg='.($current_page+1).'" class="next-arrow"><i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
								';
							}
							$pagination .= '</div>';
                            echo $pagination;
